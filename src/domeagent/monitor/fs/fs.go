...
func NewFsInfo(context Context) (FsInfo, error) {
	...
	//log.Println("Filesystem partitions: %+v", partitions)
...
func (self *RealFsInfo) GetFsInfoForPath(mountSet map[string]struct{}) ([]Fs, error) {
	...
	for device, partition := range self.partitions {
		...
		if mountSet == nil || (hasMount && !hasDevice) {
			...
			if err != nil {
				log.Printf("Statvfs failed. Error: %v", err)
			} else {
...
func getDiskStatsMap(diskStatsFile string) (map[string]DiskStats, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			log.Printf("not collecting filesystem statistics because file %q was not available", diskStatsFile)
