...
func readInt64(dirpath string, file string) uint64 {
	...
	if err != nil {
		log.Printf("raw driver: Failed to parse int %q from file %q: %s", out, path.Join(dirpath, file), err)
		return 0
	}
...
func (self *rawContainerHandler) GetSpec() (info.ContainerSpec, error) {
	...
	if self.name == "/" {
		...
		if err != nil {
			log.Printf("failed to obtain memory limit for machine container")
		...
		if err != nil {
			log.Printf("failed to obtain swap limit for machine container")
		} else {
...
func (self *rawContainerHandler) GetStats() (*info.ContainerStats, error) {
	//log.Println("raw GetStats using pid: ", os.Getpid())
...
func (self *rawContainerHandler) watchDirectory(dir string, containerName string) (bool, error) {
    ...
    defer func() {
        if cleanup {
            ...
            if err != nil {
                log.Printf("[Info] Failed to remove inotify watch for %q: %v", dir, err)
            }
        }
    }()
	...
	go func() {
		for {
			...
			case event := <-self.watcher.Event():
				...
				if err != nil {
					log.Printf("Error while processing event (%+v): %v", event, err)
				}
			case err := <-self.watcher.Error():
				log.Printf("Error while watching %q:", self.name, err)
